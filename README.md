...
| `Cmd+I` | *イタリック*トグル |
| `Cmd+E` | `インラインコード`トグル |
| `Cmd+Shift+E` | コードブロック挿入 |
| `Cmd+K` | リンク挿入 |

エディタ下部のフッターにショートカット一覧が常時表示されます。
...
日本語入力モード（IME）のままマークダウン記号を入力できます。全角で入力された記号は自動的に半角に変換されます。
...
さらに、見出し（`#`）・引用（`>`）・リスト（`-`）の後に半角スペースがなければ自動挿入されます。
...
components/
  ...
  ShortcutFooter.tsx   ← キーボードショートカット一覧フッター
...
lib/
  ...
  codemirror/
    ...
    ime-support.ts     ← 日本語IME全角→半角自動変換
```
